...
function createKendoWindow(windowID, windowTitle, openFunction, closeFunction, actions, width, height, notmodal, issysprowindow, disposeonclose, left, top, updatePlacementOff) {
  if (actions === void 0) {
    actions = [];
  }
...
function setContext(object) {
  ...
  var section_num = parseInt($(object).hasClass('data-section') ? $(object).data('section') || 0 : $(object).parent('.data-section').data('section') || 0);
  return {
    canvas_id: canvas_id,
      ...
      state.sortableDataWidgets[state.sortableDataWidgets.length] = $(elem).kendoSortable({
        ...
        start: function start() {
          ...
          $(parentElement + " .panel.draggable-data-section").each(function (_index, element) {
            $(element).closest('.data-section').removeClass('data-section').addClass('data-section-disabled');
          });
          ...
          $(parentElement + " .draggable-data-section").each(function (_index, element) {
            $(element).closest('.data-section-disabled').removeClass('data-section-disabled').addClass('data-section');
          });
...
function rowEditWindowOpen() {
  ...
  $('#rowEditWindow .btn-primary').on('click', function () {
    if (window.viewModel.selected.DisplayStyle === 9) {
      ...
      var formColumnWidths_1 = [window.viewModel.selected.ColumnLayout == "2colequal" || window.viewModel.selected.ColumnLayout == "3colleft" ? 1 : window.viewModel.selected.ColumnLayout == "2colright" || window.viewModel.selected.ColumnLayout == "3colequal" ? 3 : window.viewModel.selected.ColumnLayout == "2colleft" ? 2 : 6, window.viewModel.selected.ColumnLayout == "2colequal" || window.viewModel.selected.ColumnLayout == "3colmiddle" ? 1 : window.viewModel.selected.ColumnLayout == "2colleft" || window.viewModel.selected.ColumnLayout == "3colequal" ? 3 : window.viewModel.selected.ColumnLayout == "2colright" ? 2 : 6, window.viewModel.selected.ColumnLayout == "3colequal" ? 3 : window.viewModel.selected.ColumnLayout == "3colmiddle" || window.viewModel.selected.ColumnLayout == "3colleft" ? 6 : 1];
      window.viewModel.selected.Rows[0].Columns.forEach(function (col, idx) {
        col.ResponsiveStyle = formColumnWidths_1[idx];
      });
    }
      ...
      complete: function complete() {
        $(currentWindow + " #addDataButtonPopoverWrap, " + currentWindow + " .dataWidgetWindowButton").velocity('fadeIn', {
          ...
          complete: function complete() {
            ...
            $(currentWindow + " #addDataButtonPopoverWrap").popover('destroy');
          }
        });
      }
    });
...
function selectFieldFromSearch(item) {
  ...
  if (state.openWindow === 'dataWindow') {
    ...
    $('#addDataButtonPopoverWrap').popover('destroy'); // var targetParentWidgetDisplayStyle = SYSPRO_VB.targetParentWidget.DisplayStyle;
...
function selectToolbarFieldFromSearch(item) {
  ...
  if (state.openWindow === 'toolbarWindow') {
    ...
    $('#addtoolbarWidgetButton').removeClass('disabled');
    $('#addtoolbarWidgetButton').prop('disabled', false);
    $('#addToolbarWidgetButtonPopoverWrap').popover('destroy'); // var targetParentWidgetDisplayStyle = state.targetParentWidget.DisplayStyle;
...
function dataEditWindowOpen() {
  ...
  $('#dataEditWindow #editDataWidgetSaveButton').on('click', function () {
    var currentDataWidgetHTML = window.viewModel.selected.TypeName === 'LinkWidget' || window.viewModel.selected.HasSmartTag === true ? $("[data-guid=\"" + window.viewModel.selected.Id + "\"]").closest('a') : $("[data-guid=\"" + window.viewModel.selected.Id + "\"]");
...
function init() {
  ...
  createKendoWindow('rowEditWindow', 'Edit layout widget', rowEditWindowOpen, rowEditWindowClose, null, null, null, null, null, null, null, null, true); // Add data widget Kendo Window

  createKendoWindow('dataWindow', 'Add new data widget', dataWindowOpen, dataWindowClose, null, null, null, null, null, null, null, null, true); // Edit data widget Kendo Window

  createKendoWindow('dataEditWindow', 'Edit data widget', dataEditWindowOpen, dataEditWindowClose, null, null, null, null, null, null, null, null, true); // Add tile widget Kendo Window

  createKendoWindow('tileWindow', 'Add tile', tileWindowOpen, tileWindowClose, null, null, null, null, null, null, null, null, true); // Add tile widget Kendo Window

  createKendoWindow('tileEditWindow', 'Edit tile', tileEditWindowOpen, tileEditWindowClose, null, null, null, null, null, null, null, null, true); // Add toolbar widget Kendo Window

  createKendoWindow('toolbarWindow', 'Add toolbar item', toolbarWindowOpen, toolbarWindowClose, null, null, null, null, null, null, null, null, true); // Add toolbar widget Kendo Window

  createKendoWindow('toolbarEditWindow', 'Edit toolbar item', toolbarEditWindowOpen, toolbarEditWindowClose, null, null, null, null, null, null, null, null, true); // New Layout Widget Window

  createKendoWindow('rowWindow', 'Add new layout widget', rowWindowOpen, rowWindowClose, null, null, null, null, null, null, null, null, true);
  sysproInterop.getModel('', function (dataout) {
    ...
    $(document).on('click', '.add-data-section', function (e) {
      ...
      if (context.targetLayoutWidget.DisplayStyle === 4) {
        ...
        $('#dataWindow .drilldown-loading-cover').show();
        context.section_num = $(e.currentTarget).closest('[data-section]').data('section');
        loadIconJSON('base', '#dataWindow');
        $('#dataWindow').data('kendoWindow').setOptions({
          context: context
    ...
    $('.window-content').off('click', '.clear-icon-selection', clearIconSelection).on('click', '.clear-icon-selection', clearIconSelection);
